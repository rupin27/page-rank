...
fh = open(os.path.join(sys.path[0].replace('/src', ''), 'inlinks.txt'), 'w', encoding='utf-8')
fh2 = open(os.path.join(sys.path[0].replace('/src', ''), 'pageRank.txt'), 'w', encoding='utf-8')
...
def inlinkFreqList(lst, numLinks):
    dict = createDict(lst)
    sort_by_val = {}
    ...
    for i in range(numLinks):
        fh.write(link[i])
        fh.write(' ' + str(i+1) + ' ' + str(count[i]) + '\n')

def pageRankVal(dict, numLinks):
    sort_by_val = {}
    sortedKeys = sorted(dict, key=dict.get) 
    for links in sortedKeys:
        sort_by_val[links] = dict[links]
    ...
    for i in range(numLinks):
        fh2.write(link[i])
        fh2.write(' ' + str(i+1) + ' ' + str(count[i]) + '\n')

def pageRankList(page, link, lam, tau):
    curPR = createDict3(page, link)
    resultPR = createDict3(page, link)
    mapped = createDict2(page, link)
    for k in curPR:
...
inlinkFreqList(target, 100)
pageRank = pageRankList(source, target, lambd, tau)
pageRankVal(pageRank, 100)
f.close()
fh.close()
fh2.close()
